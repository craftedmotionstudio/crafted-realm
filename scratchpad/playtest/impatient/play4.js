...
const fs = require('fs');
const OUT = process.env.PT_OUT || 'scratchpad/playtest/impatient/run5-full';
...
(async () => {
  const p = await Player.launch({port: 8779, profile: 'impatient4', out: OUT});
  await p.login();
  ...
  say('[RUN5] seconds', rep.seconds, 'zone', v.zone, 'sailed', sailed, 'complete', v.tutorialComplete, 'findings', p.findings.length, 'errors', rep.errors.length);
  await p.close();
})().catch(e => { say('RUN5 ERROR ' + e.stack); process.exit(1); });
