...
const fs = require('fs');
const OUT = process.env.PT_OUT || 'scratchpad/playtest/impatient/run3-full';
...
(async () => {
  const p = await Player.launch({port: 8779, profile: 'impatient2', out: OUT});
  await p.login();
  ...
  await phase(p, 'D chop (click far, click again, walk off mid-chop)', async () => {
  ...
  say('[RUN3] seconds', rep.seconds, 'zone', v.zone, 'complete', v.tutorialComplete, 'findings', p.findings.length, 'errors', rep.errors.length);
  await p.close();
})().catch(e => { say('RUN3 ERROR ' + e.stack); process.exit(1); });
