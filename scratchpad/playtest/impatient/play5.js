...
const fs = require('fs');
const OUT = process.env.PT_OUT || 'scratchpad/playtest/impatient/run6-full';
...
async function rush(p, x, z, opts){
  ...
  let r = await p.walkTo(x, z, Object.assign({maxLegs: 24}, opts || {}));
...
(async () => {
  const p = await Player.launch({port: 8779, profile: 'impatient5', out: OUT});
  const rawLocate = p.locate.bind(p);
  let bannerHazards = 0;
  ...
  say('[RUN6] seconds', rep.seconds, 'zone', v.zone, 'sailed', sailed, 'complete', v.tutorialComplete, 'findings', p.findings.length, 'errors', rep.errors.length);
  await p.close();
})().catch(e => { say('RUN6 ERROR ' + e.stack); process.exit(1); });
