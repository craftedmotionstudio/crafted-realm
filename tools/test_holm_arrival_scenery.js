...
const input={layout:read('docs/rebuild/holm-overhaul/arrival-layout.json'),envelopes:read('docs/rebuild/holm-overhaul/guide-house-collision-envelopes.json'),terrain:read('.studio-workspaces/holm-overhaul-terrain-v1/working/assets/world/authoring/holm-overhaul.terrain.bundle.json'),placement:read('docs/rebuild/holm-overhaul/arrival-landscape.json'),measurement:read('.studio-workspaces/holm-arrival-landscape-measure-v1/candidates/measured.json')};
...
assert.strictEqual(JSON.stringify(input),original);assert.deepStrictEqual(Compiler.compile(input),out);assert.strictEqual(out.placements.length,17);assert.strictEqual(out.envelopes.blockers.length,input.envelopes.blockers.length+34);
...
for(const p of out.placements){
 ...
 for(const surface of ['exterior','ground']){const b=out.envelopes.blockers.find(q=>q.id===p.id+(surface==='ground'?'.ground':''));assert.strictEqual(b.surface,surface);assert.strictEqual(b.x0,p.worldBounds.min[0]-input.layout.building.world.x);assert.strictEqual(b.z1,p.worldBounds.max[2]-input.layout.building.world.z)}
}
...
console.log('[HOLM_ARRIVAL_SCENERY_TEST] 17 authored placements, 34 translated blockers, 7 actual semantic roots, 4 door graphs, 16 preserved route legs, immutable classic/CommonJS parity; '+rejected+' invalid inputs rejected. No visual or continuous animation acceptance.');
