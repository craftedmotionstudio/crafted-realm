...
const args=process.argv.slice(2),QUICK=args.includes('--quick'),PVP_ONLY=args.includes('--pvp-only'),TAG=args.find(a=>!a.startsWith('--'))||'latest';
  ...
  for(const L of points){
    ...
    for(const style of ['melee','ranged','magic']){
      ...
      for(let f=0;f<N_PVM;f++){
        ...
        const r=h.until(()=>n.dead||h.P.dead||h.P.hp<=0,1500);
      ...
      rows.push({L,style,monster:mon,monsterLevel:NPC[mon].level,weapon:lo.weapon,fights:N_PVM,wins,ttkTicks:+mean(ttk).toFixed(1),ttkSec:+(mean(ttk)*0.6).toFixed(1),
...
const B=pvp();console.log('PvP done '+((Date.now()-t0)/1000).toFixed(0)+' s');
...
md+='## A. PvM time to kill (client engine, no food, '+N_PVM+' fights each)\n\n| level | style | weapon | monster (level) | kills | avg kill time | damage taken | XP/hour (excl. HP) |\n|---|---|---|---|---|---|---|---|\n';
A.forEach(r=>md+=`| ${r.L} | ${r.style} | ${r.weapon} | ${r.monster} (${r.monsterLevel}) | ${r.wins}/${r.fights} | ${r.ttkSec} s (${r.ttkTicks} ticks) | ${r.dmgTaken} of ${r.hp} | ${r.xpPerHour} |\n`);
