...
const root=path.resolve(__dirname,'..'),id='holm-arrival-package-v3';
...
const landscapeBase='.studio-workspaces/holm-arrival-landscape-measure-v1/candidates/';
const measured=JSON.parse(fs.readFileSync(path.join(root,landscapeBase+'measured.json'),'utf8'));
const roots={oak:'ArrivalOak',hazel:'ArrivalHazel',fieldstones:'ArrivalFieldstones',wall:'LandingGardenWall',bench:'LandingOakBench',waypost:'LandingWaypost',cargo:'LandingCargoCrate'};
...
for(const [id,m] of Object.entries(measured.assets)){
 const target='assets/models/holm_arrival_'+id+'_v1.glb';
 const model=add(target,m.file);if(model.sha256!==m.sha256)throw Error('Measured landscape bytes changed: '+id);
 assets.push({id,ownerId:'holm_landscape_'+id,model,authoring:['oak','hazel','fieldstones'].includes(id)?gardenSource:landingSource,parts:[roots[id]]});
}
sources.landscapePlacement=add(authoring+'holm-arrival.landscape.json','docs/rebuild/holm-overhaul/arrival-landscape.json');
sources.landscapeMeasurement=add(authoring+'holm-arrival.landscape-measure.json',landscapeBase+'measured.json',d=>{for(const [id,m] of Object.entries(d.assets))m.file='assets/models/holm_arrival_'+id+'_v1.glb';return d});
