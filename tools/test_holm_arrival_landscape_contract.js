...
test('existing study has all 18 authored placements (incl. Lantern Keeper statue); synthetic bounds do not claim asset proof',()=>{const f=fixture();f.data=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/rebuild/holm-overhaul/arrival-landscape.json'),'utf8'));f.layout=JSON.parse(fs.readFileSync(path.join(__dirname,'../docs/rebuild/holm-overhaul/arrival-layout.json'),'utf8'));f.templates={};for(const p of f.data.placements)f.templates[p.asset]={localBounds:{min:[-.01,0,-.01],max:[.01,.1,.01]}};const out=Contract.compile(f);assert.equal(out.placements.length,18);assert.equal(out.evidence.sourceBytesVerified,false);assert.deepEqual(out.placements.map(p=>[p.asset,p.transform.x,p.transform.z,p.transform.scale,p.transform.rotation]),f.data.placements.map(p=>[p.asset,p.x,p.z,p.scale,p.rotation]))});
