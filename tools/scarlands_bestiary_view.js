...
(function(){
 ...
 window.bvPose = function(id, clip, frame){ var m = models[id]; if (!m) throw Error('no model ' + id); var c = m.clips[clip]; if (!c) throw Error(id + ' has no clip ' + clip);
   m.mixer.stopAllAction(); var a = m.mixer.clipAction(c); a.reset(); a.play(); a.paused = false; m.mixer.setTime(Math.min(c.duration, frame / 30)); render(); return c.duration * 30; };
