#!/usr/bin/env node
/* Build the Scarlands art kit v1 end to end (W2/W3, 2026-09-26):
 *  1. the old-school texture kit (tools/build_oldschool_textures.js, now with the Scarlands textures);
 *  2. the pieces in Blender (tools/blender/build_scarlands_kit_v1.py -> .studio-workspaces/scarlands-kit-v1/working);
 *  3. UVs + kit textures (tools/blender/apply_oldschool_textures.py with docs/rebuild/scarlands/scarlands-kit.textures.json
...
const ROOT = path.resolve(__dirname, '..'), WS = '.studio-workspaces/scarlands-kit-v1', BLENDER = 'C:/Program Files/Blender Foundation/Blender 4.5/blender.exe';
...
console.log(run(BLENDER, ['-b', '--python-exit-code', '1', '--python', 'tools/blender/build_scarlands_kit_v1.py']).split(/\r?\n/).find(l => l.startsWith('[SCARLANDS KIT]')));
const spec = 'docs/rebuild/scarlands/scarlands-kit.textures.json';
...
const manifest = {
  schema: 'crafted-realm-scarlands-kit-v1', version: 1, name: 'The Scarlands art kit',
  ...
  conventions: {
    ...
    cells: 'collision cells (i, j): i east 0..w-1, j north 0..d-1 (j = 0 south row), unrotated; blocked = solid tile, water = unwalkable floor projectiles cross, walls = [i, j, side] edge walls, cut = lower the ground under these cells by depth (the Ditch), walk = walkable deck height over cut cells',
