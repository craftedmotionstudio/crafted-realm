...
async function playOnce(browser,n){
  ...
  const rec={run:n,profile,status,minutes:+((Date.now()-t0)/60000).toFixed(1),lessons:Object.keys(per).length,perLessonSeconds:per,errors:errors.slice(0,5),note,skillTools,at:new Date().toISOString()};
