"""Extract sampled cardinal routes from frozen evaluated lodge and a local Studio apron.
...
OUT=ROOT/'.studio-workspaces/holm-quest-navigation-v1/candidates'
SHA='6de1fd292bfdd0f3dd51bf0031297f8905eb72ff19a8ed9be759b7f4f738140b'
assert hashlib.sha256((BASE/'lodge.glb').read_bytes()).hexdigest()==SHA
assert (BASE/'contract.json').exists() and (BASE/'lodge.blend').exists()
bpy.ops.wm.open_mainfile(filepath=str(BASE/'lodge.blend'))
# Measured open-door pose only; closed-door routing is not certified.
bpy.context.scene.frame_set(60)
deps=bpy.context.evaluated_depsgraph_get()
verts=[]; faces=[]; labels=[]; support_faces=[]; support_labels=[]
for ob in bpy.context.scene.objects:
 if ob.type!='MESH' or not ob.name.startswith('Lodge_'):continue
 ...
 eo.to_mesh_clear()
# Flat local Studio apron; this is NOT the island terrain or a world placement.
verts.extend([Vector((-8,-8,-.02)),Vector((8,-8,-.02)),Vector((8,8,-.02)),Vector((-8,8,-.02))])
o=len(verts)-4
for tri in [(0,1,2),(0,2,3)]:
 f=tuple(o+i for i in tri);faces.append(f);labels.append('StudioApron');support_faces.append(f);support_labels.append('StudioApron')
...
start=closest(.5,-.02,3.5)
seen={start['id']};queue=deque(seen)
while queue:
 for v in links[queue.popleft()]:
  if v not in seen:seen.add(v);queue.append(v)
targets=[]
for ident,label,x,y,z in [('entrance','Lodge entrance',.5,0,1.5),('board','Quest board',-3.5,0,2.5),('map','Map table',2.5,0,1.5),('bay','Reading bay',4.5,0,-1.5),('guest','Upstairs guest room',-3.5,3.3,.5)]:
 node=closest(x,y,z,lambda n:abs(n['y']-y)<.35 and abs(n['x']-x)<.01 and abs(n['z']-z)<.01)
...
data={'schema':'holm-keep-navigation-v1','modelSha256':SHA,'modelUrl':'../.studio-workspaces/holm-quest-lodge-v2/candidates/lodge.glb','surfaceContext':'local Studio apron only; no island placement','doorPoseTimeSeconds':59/24,'avatar':{'radius':R,'height':HEIGHT},'edgeSampleSpacing':SPACING,'nodes':nodes,'links':links,'profiles':profiles,'startId':start['id'],'targets':targets,'report':report}
...
(OUT/'REPORT.md').write_text('# Quest Lodge navigation extraction\n\n'+json.dumps(report,indent=2)+'\n\nTargets:\n'+json.dumps(targets,indent=2)+'\n',encoding='utf-8')
print('[QUEST_NAVIGATION]',json.dumps(report),flush=True)
print('[QUEST_NAVIGATION_TARGETS]',json.dumps(targets),flush=True)
