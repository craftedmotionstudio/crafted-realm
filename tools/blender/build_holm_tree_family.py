...
OUT=ROOT/'.studio-workspaces/holm-tree-family-v1/candidates'
...
def texture(name,base,kind):
 ...
 for y in range(n):
  for x in range(n):
   ...
   if kind=='leaf': noise+=.055*math.sin(x*1.4+y*.5)*math.cos(y*1.8)
...
class Mesh:
 ...
 def crown(self,c,s,seed):
  ...
  for j,(h,r) in enumerate([(-.5,.28),(-.32,.88),(.05,1),(.33,.72),(.48,.22)]):
   for i in range(n):
    a=math.tau*i/n+.12*j;rr=r*(1+.15*math.sin(a*3+seed))*rng.uniform(.9,1.1)
...
for i,(c,s) in enumerate([((.42,-.1,2.7),(1.13,.65,.75)),((1.43,.4,3.45),(1.37,.85,.77)),((.37,-.25,4.00),(1.15,.77,.73)),((1.70,-.03,4.72),(1.35,.90,.85)),((2.05,.1,5.05),(.82,.69,.68))]):
 t.branch([(.45+i*.19,0,c[2]-.55),(c[0]-.3,c[1],c[2]-.15),c],[.095,.05,.015]);l.crown(c,s,330+i)
...
for ob,habitat in assets:
 ...
 print('[HOLM_TREE_FAMILY]',ob.name,tris,'tris; finite rest + 41 sampled bounds; exported Breeze endpoints match')
scene.frame_set(1);bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'tree-family.blend'))
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n',encoding='utf8')
print('[HOLM_TREE_FAMILY] 5/5 asset structural gates PASS; visual and game integration pending')
