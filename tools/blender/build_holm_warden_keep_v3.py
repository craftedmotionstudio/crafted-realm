...
prefix=prefix.replace('holm-warden-keep-v1/candidates','holm-warden-keep-v3/candidates')
...
contract=json.loads((BASE/'keep.contract.json').read_text(encoding='utf-8'));contract.update(schema='holm.warden-keep.v3',boundsBlender=bounds,triangleCount=tris,meshCount=len(production),meshNames=[o.name for o in production],towers=TOWERS)
