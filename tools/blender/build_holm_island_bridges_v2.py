"""Tutor's Holm creek crossings v2 (2026-09-25 world fixes, owner play-test: "the bridge is underwater ... it walks
under in the water"). Same original designs as v1 (build_holm_island_bridges_v1.py: a timber teaching bridge and an
arched stone village bridge), rebuilt to the re-measured deck tiles: HolmIslandNav.bridgeFrom now counts the creek banks
under the water line as water, so each deck spans the whole carved channel lip to lip and sits `rise` over the higher
bank, clear of the drawn creek surface (waterY in the data). The timber posts carry their rope lashing at the real
waterline; the stone bridge is a humpback whose arch stands clear of the water.
Data: argv after "--" or the default docs/rebuild/holm-overhaul/island-bridges.json. Each GLB's origin is the bridge's
world centre at y=0, so the runtime places it at (cx, 0, cz); deck heights are world heights. Both run east-west
(along game x). Game (x,y,z) -> Blender (x,-z,y).
Run: blender -b -P tools/blender/build_holm_island_bridges_v2.py -- [bridges.json]
"""
...
def build(br):
 ...
 dy=br['deckY'];e0,e1=br['endY'];bed=br['bedY'];w0,w1=br['waterY'];rng=random.Random(br['id'])
 assert dy-max(w0,w1)>=.25,'deck must clear the water'
 def water(x):   # drawn creek surface along the bridge (first and last deck-tile centres, linear between)
  return w0+(w1-w0)*min(1,max(0,(x-(xw0+.5))/max(.5,(xw1-.5)-(xw0+.5))))
 ...
 if br['id'].startswith('timber'):
  ...
  for px in posts:
   ...
   for s in (-.82,.82):
    ...
    if xw0-.01<=px<=xw1+.01:box('BridgePostWrap',px-.08,px+.08,water(px)-.02,water(px)+.1,cz+s-.08,cz+s+.08,rope)   # rope lashing at the waterline
  ...
  am=(xw0+xw1)/2;ar=(xw1-xw0)/2+.05;apex=dy-.3;n=12
  assert apex-max(w0,w1)>=.2,'the arch crown must stand clear of the water'
  for i in range(n+1):
...
for r in report:
 for ext in ('.glb','.blend'):r['sha256'+ext.replace('.','_')]=hashlib.sha256((OUT/(r['id']+ext)).read_bytes()).hexdigest()
(OUT/'manifest.json').write_text(json.dumps({'schema':'holm-island-bridges-models-v1','source':SRC.relative_to(ROOT).as_posix(),'sourceSha256':hashlib.sha256(SRC.read_bytes()).hexdigest(),
 'deck':{b['id']:{'tiles':b['tiles'],'deckY':b['deckY'],'waterY':b['waterY']} for b in DATA['bridges']},'bridges':report},indent=1))
