...
OUT=ROOT/'.studio-workspaces/holm-tree-family-v3/candidates'
...
# v3 oak (owner review 6, 2026-09-24): "they don't have any branches that stick out like the references".
# A leaning trunk forks into five limbs that reach well out; each limb carries its OWN smaller crown with
# gaps between them so the limbs read, and bare twigs poke past every crown's silhouette. Two short bare
# lower limbs break the trunk line. Same materials, textures and Breeze clip as v2.
t=Mesh();l=Mesh();t.branch([(0,0,0),(.12,0,.9),(-.12,.1,1.9),(.05,.1,2.5)],[.45,.3,.24,.2])
limbs=[((-1.8,.25,3.5),(1.3,1.05,1.1)),((1.7,.45,3.75),(1.35,1.1,1.2)),((-.4,-1.5,4.05),(1.2,1.1,1.15)),((-.5,1.4,4.45),(1.25,1.05,1.1)),((.5,-.12,5.05),(1.25,1.1,1.2))]
for i,(c,s) in enumerate(limbs):
 mid=(c[0]*.5,c[1]*.5,2.5+(c[2]-2.5)*.55)
 t.branch([(.02,.08,2.3),mid,c],[.2,.13,.06])
 l.crown(c,s,100+i)
 # one leafy twig breaks out past the crown edge and one short bare twig beside it: branches show at the rim
 out=(c[0]-mid[0],c[1]-mid[1]);n=math.hypot(*out) or 1;out=(out[0]/n,out[1]/n)
 side=(-out[1],out[0]);r=max(s)*.9
 tip=(c[0]+out[0]*(r+.35)+side[0]*.3,c[1]+out[1]*(r+.35)+side[1]*.3,c[2]-.15)
 t.branch([(c[0]+out[0]*.3,c[1]+out[1]*.3,c[2]-.3),((c[0]+tip[0])/2,(c[1]+tip[1])/2,c[2]-.28),tip],[.06,.04,.02],5)
 if i%2==0:l.mass(tip,(.42,.38,.36),300+i)
 bare=(c[0]+out[0]*(r+.2)-side[0]*.45,c[1]+out[1]*(r+.2)-side[1]*.45,c[2]+.35)
 t.branch([(c[0]+out[0]*.4,c[1]+out[1]*.4,c[2]),bare],[.035,.012],5)
for k,(a,z,ln) in enumerate([(.9,1.6,1.05),(3.9,1.95,.85)]):
 tip=(math.cos(a)*ln,math.sin(a)*ln,z+.7)
 t.branch([(0,0,z),(math.cos(a)*ln*.55,math.sin(a)*ln*.55,z+.3),tip],[.1,.06,.03],6)

for i in range(5):
 a=math.tau*i/5+.3;t.branch([(math.cos(a)*.85,math.sin(a)*.85,.03),(math.cos(a)*.25,math.sin(a)*.25,.22),(0,0,.6)],[.02,.13,.24])
...
for ob,habitat in assets:
 ...
 print('[HOLM_TREE_FAMILY_V3]',ob.name,tris,'tris; finite rest + 41 sampled bounds; exported Breeze endpoints match')
scene.frame_set(1);bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'tree-family.blend'))
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n',encoding='utf8')
print('[HOLM_TREE_FAMILY_V3] 5/5 asset structural gates PASS; visual and game integration pending')
