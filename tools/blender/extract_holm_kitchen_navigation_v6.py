"""Extract sampled cardinal routes from frozen evaluated bakehouse and actual staged terrain.
...
BASE=ROOT/'.studio-workspaces/holm-kitchen-wings-v8/candidates'
OUT=ROOT/'.studio-workspaces/holm-kitchen-navigation-v6/candidates'
SHA=hashlib.sha256((BASE/'kitchen-character.glb').read_bytes()).hexdigest()   # v6: measured on kitchen wings v8 (the 2026-09-25 redesign)
assert hashlib.sha256((BASE/'kitchen-character.glb').read_bytes()).hexdigest()==SHA
assert (BASE/'kitchen-character.contract.json').exists() and (BASE/'kitchen-character.blend').exists()
...
for ob in bpy.context.scene.objects:
 if ob.type!='MESH' or not ob.name.startswith('Kitchen_'):continue
...
for wz in range(59,75):
 for wx in range(36,54):
  offset=len(verts)
  for x,z in [(wx,wz),(wx+1,wz),(wx,wz+1),(wx+1,wz+1)]:
   verts.append(Vector((x-44,-(z-67),terrain['heights'][z*stride+x]-4.07)))
...
start=closest(2.5,-.07,2.5)
seen={start['id']};queue=deque(seen)
while queue:
 for v in links[queue.popleft()]:
  if v not in seen:seen.add(v);queue.append(v)
targets=[]
for ident,label,x,y,z in [('entrance','Courtyard entrance',.5,0,1.5),('oven','Oven working stance',-3.5,0,-2.5),('prep','Kneading table (Cook Hettie)',-2.5,0,.5),('pantry','Flour bin',4.5,0,-3.5),('loft','Sleeping loft',-2.5,3.3,-2.5),
  ('buckets','Bucket rack',-1.5,0,1.5),('water','Water butt',-.5,0,2.5),('dough','Dough trough',-1.5,0,.5),('recipe','Recipe board',.5,0,-1.5)]:
...
data={'schema':'holm-keep-navigation-v1','modelSha256':SHA,'terrainSha256':hashlib.sha256(terrain_bytes).hexdigest(),'placement':{'x':44,'y':4.07,'z':67},'avatar':{'radius':R,'height':HEIGHT},'edgeSampleSpacing':SPACING,'nodes':nodes,'links':links,'profiles':profiles,'startId':start['id'],'targets':targets,'report':report}
...
(OUT/'REPORT.md').write_text('# Bakehouse navigation extraction\n\n'+json.dumps(report,indent=2)+'\n\nTargets:\n'+json.dumps(targets,indent=2)+'\n',encoding='utf-8')
print('[KITCHEN_NAVIGATION]',json.dumps(report),flush=True)
print('[KITCHEN_NAVIGATION_TARGETS]',json.dumps(targets),flush=True)
