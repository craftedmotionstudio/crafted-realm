"""Reversible Blender keep v2: measured tower switchbacks and open lookouts.
...
prefix=prefix.replace('holm-warden-keep-v1/candidates','holm-warden-keep-v2/candidates')
prefix=prefix.replace("tower('East turret',9.3,7,2.3,6.7,'west',[6.15,7.85],7,True)","tower('East turret',9.65,7,2.65,6.7,'west',[6.15,7.85],7,False)")
...
for name,cx,cy,ap,h,count in [('High watch',-7,12,3,10.2,3),('East turret',9.65,7,2.65,6.7,2)]:
...
contract=json.loads((BASE/'keep.contract.json').read_text(encoding='utf-8'));contract.update(schema='holm.warden-keep.v2',boundsBlender=bounds,triangleCount=tris,meshCount=len(production),meshNames=[o.name for o in production],towers=TOWERS)
...
contract['deviations']=['East turret expanded from apothem2.3 at(9.3,7) to apothem2.65 at(9.65,7), keeping west doorwayx7. Parent authorized measured expansion.','East turret conical cap replaced by open crenellated lookout.','Tower apertures include mid-flight turns, avoiding low overhead slabs. Tower upper spaces are circulation/lookout galleries around the open stairs.']
...
(OUT/'REPORT.md').write_text(f'''# Keep v2 tower access candidate\n\nActual Blender source and exports, unpublished. {tris} triangles, {primitives} primitives.\n\nBoth towers have stacked 1.25 m cardinal switchback flights, 1.2 m deep landings, open floor apertures and lookout parapets. The east tower expansion is recorded in the contract. {len(ACCESS)} tower treads plus the preserved 16 hall treads were measured from actual mesh vertices. {head_samples} vertical scene rays and {door_samples} doorway crossing rays and {entry_samples} horizontal stair-entry rays passed; these are discrete geometry samples, not runtime avatar validation.\n\nFull-world placement, navigation/player integration, animation, reference review and Safe Publish remain unimplemented. The east wallwalk still has its closed north stop; turret entry is on the ground.\n''',encoding='utf-8')
print('[WARDEN_KEEP_V2] PASS',tris,primitives,door_samples,head_samples,contract['glb']['sha256'])
