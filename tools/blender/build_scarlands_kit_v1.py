"""The Scarlands art kit v1 (W2/W3, 2026-09-26): the burned Wilderness north of the Wilderness Ditch.
...
Run: blender -b --python tools/blender/build_scarlands_kit_v1.py
"""
...
WS = ROOT / '.studio-workspaces/scarlands-kit-v1/working'
...
# ---------- the Ditch trench kit (a 2-tile-deep E-W trench; clients carve the ground under 'cut' cells) ----------
DEPTH = 1.6
def trench_bed(p, x0, x1, y0, y1):
    p.plate([(x0, y0, -DEPTH), (x1, y0, -DEPTH), (x1, y1, -DEPTH), (x0, y1, -DEPTH)], M['ashbed'])

def stone_face(p, x0, x1, y, facing):
    """Stone revetment down the south bank (facing north), from the lip to the bed, coursed blocks."""
    z = -DEPTH
    row = 0
    while z < -0.02:
        ...
        while x < x1 - 0.02:
            ...
            if b - a > 0.08:
                p.box((b - a - 0.03, 0.3, 0.38), ((a + b) / 2, y + facing * 0.08 + 0.02 * p.rng.uniform(-1, 1), z + 0.19), (0, 0, 0), M['stone'] if p.rng.random() > .3 else M['stone_dark'])
            x += 0.5
        z += 0.4
        row += 1

def earth_bank(p, x0, x1, y):
    """The raw north bank: a scorched earth slope from the lip down to the bed."""
    p.plate([(x0, y, 0.0), (x0, y - 0.35, -DEPTH), (x1, y - 0.35, -DEPTH), (x1, y, 0.0)], M['soil'], both=True)

def stake(p, x, y):
    """A sharpened stake set in the south bank, leaning out over the trench toward the north."""
    p.limb((x, y - 0.1, -0.1), (x + p.rng.uniform(-0.05, 0.05), y + 0.55, 1.0), 0.08, 0.012, 5, M['char'])

p = Piece('ditch_straight', 1, 2, ['ditch'], rotations=(0, 2), about='One tile of the Wilderness Ditch (1 wide, 2 deep): stone-faced south bank with a sharpened stake leaning north, raw north bank.')
trench_bed(p, -0.5, 0.5, -1.0, 1.0)
stone_face(p, -0.5, 0.5, -1.0, 1)
earth_bank(p, -0.5, 0.5, 1.0)
stake(p, -0.25 + p.rng.uniform(-0.05, 0.05), -1.02)
stake(p, 0.25 + p.rng.uniform(-0.05, 0.05), -1.02)
...
earth_bank(p, -0.5, 0.5, 1.0)
for k in range(4):
    p.box((0.3, 0.5, 0.38), (-0.42, -0.75 + k * 0.5, -DEPTH + 0.19), (0, 0, 0), M['stone_dark'])
    p.box((0.3, 0.5, 0.38), (-0.42, -0.75 + k * 0.5, -DEPTH + 0.59), (0, 0, 0), M['stone'])
    p.box((0.3, 0.5, 0.38), (-0.42, -0.75 + k * 0.5, -DEPTH + 0.99), (0, 0, 0), M['stone_dark'])
    p.box((0.3, 0.5, 0.36), (-0.42, -0.75 + k * 0.5, -DEPTH + 1.38), (0, 0, 0), M['stone'])
p.water = all_cells(p); p.cut = all_cells(p); p.cut_depth = DEPTH; done(p)
...
for k in range(4):
    for zz in range(4):
        p.box((0.3, 0.5, 0.38), (0.92, -0.75 + k * 0.5, -DEPTH + 0.19 + zz * 0.4), (0, 0, 0), M['stone'] if (k + zz) % 2 else M['stone_dark'])
p.water = all_cells(p); p.cut = all_cells(p); p.cut_depth = DEPTH; done(p)
...
courses(p, -0.5, 0.5, 0.5, 0.62, 0.25, seed=31, thick=0.34)
done(p)
