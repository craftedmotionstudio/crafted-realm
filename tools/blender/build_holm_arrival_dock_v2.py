"""Tutor's Holm arrival dock v2 (owner, 2026-09-25): v1's deck, rails, ropes and bollards unchanged; the
...
OUT=ROOT/'.studio-workspaces/holm-arrival-dock-v2/candidates'
...
PALETTE=[('Sun-worn oak',(.43,.32,.21)),('Pale plank edges',(.50,.39,.26)),('Dark structural oak',(.29,.22,.15)),('Tarred iron',(.16,.18,.17)),('Hemp rope',(.53,.44,.29)),('Boat sackcloth',(.69,.61,.44)),('Boat sheer paint',(.31,.43,.37))]
mats=[]
...
BEARERS=[-3.72,-1.24,1.24,3.72];PILE_X=1.64;PILE_R=.16
...
for x in [-PILE_X,PILE_X]:
 ...
 for za,zb in zip(BEARERS,BEARERS[1:]):
  for lo,hi in ((za,zb),(zb,za)):
   a=Vector((bx,-1.75,lo+(.06 if hi>lo else -.06)));b=Vector((bx,-.66,hi-(.06 if hi>lo else -.06)))
   d=(b-a).normalized()
   box_pts=[a-d*.08,b+d*.08]
   beam('DockStructure',tuple(box_pts[0]),tuple(box_pts[1]),.07,2)
  for zz in (za,zb):
   for yy in (-1.75,-.66):rod('DockIron',(bx-out*.05,yy,zz),(bx+out*.06,yy,zz),.03,3,n=6)       # bolt heads through brace and pile
# Guard posts are wholly outside the clear x[-1.5,1.5] strip.
for x in [-1.64,1.64]:
 for z in [-4.20,-1.30,1.30,4.43]:
  box('DockRails',x-.11,x+.11,-.34,1.0,z-.065,z+.065,2,True)   # v2: down onto the edge joist
...
# bottom boards, thwarts, stern seat
box('DockBoatHull',-.2,.2,-.13,-.11,-1.1,1.1,1)
for z,w in ((-.35,.2),(.55,.2)):
 b=half_beam(z)-.03;box('DockBoatHull',-b,b,.12,.16,z-w/2,z+w/2,0)
b=half_beam(-1.2)-.03;box('DockBoatHull',-b,b,.1,.14,-1.4,-1.05,0)
# oars resting on the thwarts, blades forward; oarlock pins on the gunwales
for x in (-.17,.17):
 rod('DockBoatFittings',(x,.19,-1.3),(x,.19,1.0),.026,1,n=6)
 box('DockBoatFittings',x-.065,x+.065,.18,.2,1.0,1.42,1)
for side in (-1,1):
...
for i,(r,y) in enumerate(((.17,-.08),(.15,-.04),(.13,0))):
...
rod('DockBoatFittings',(0,-.1,-.85),(0,.25,-.85),.17,5,n=7);rod('DockBoatFittings',(0,.25,-.85),(0,.33,-.85),.06,4,n=6)
box('DockBoatFittings',-.34,-.02,-.11,.18,-.72,-.42,1);box('DockBoatFittings',-.35,-.01,.05,.08,-.73,-.41,2)
# boat pose beside the east side of the deck, bow toward the bollard at z 2.65
PIV=Vector((2.62,-1.03,1.55));YAW=math.radians(-4);LIST=math.radians(4)
def to_dock(p):
...
assert tris<12000 and len(meshes)<=12 and len(mats)<=8
...
assert primitive_count<=40 and len(gltf['materials'])<=8
