...
(function(){
  ...
  check('the v3 preview keeps the owner-directed 2004 look (reviews 2+5): blended HSL + bright baked light, earth cliffs, per-tile palette shading, no grid, far black edge, clickable decks',
    /function prepare2004\(b\)\{/.test(r3)&&/return 128\*\(\.48\+\.64\*d\);/.test(r3)&&/var CLIFF=\[/.test(r3)&&/noGrid/.test(ui4)&&/tileRnd/.test(r3)&&/new THREE\.MeshBasicMaterial\(\{vertexColors:true\}\)/.test(r3)&&
    /hit\.name='ground';/.test(r3)&&/v3Look \? 0x000000/.test(mainSource)&&/camCtl\.dist\+18/.test(mainSource)&&
...
check('Holm teaching fires burn 150 s, tutorial menus hide Mark Tile, the net says where it is walking, and the departure banner survives graduation',
  /fire\.userData\.ttl=\(typeof CRWorldMode!=='undefined'&&\/\^tutors-holm-\(v2\|v3\|arrival-qa\)\$\/\.test\(CRWorldMode\.providerId\)\)\?150:65;/.test(mainSource)&&
