...
function buildCtxEntries(hit, e){
  ...
  if(hit && hit.obj){
    ...
    if(u.kind==='npc'){
      const npc=u.npc;
      if(npc && !npc.dead){
        entries.push({html:`Attack <b>${npc.t.name}</b> <span class="lv" style="color:${combatLevelColour(npc.t.level)}">(level-${npc.t.level})</span>`,
          fn:()=>{ if(typeof LocalCombat!=='undefined' && LocalCombat.ready()) LocalCombat.orderAttack(npc); else { Player.target=npc; Player.action=null; } }});
